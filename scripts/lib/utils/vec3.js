 ...
 * @returns {number}
 */
export const radian = (angle) => angle * Math.PI / 180;
...
export class Vec3 {
    ...
    static circle(vec, r, angle) {
        return Vec3.from(vec).offset(r * Math.cos(radian(angle)), 0, r * Math.sin(radian(angle)));
    }
    ...
    static direction(rot) {
        return ((vec) => new Vec3(-Math.sin(vec.y) * Math.cos(vec.x), -Math.sin(vec.x), Math.cos(vec.y) * Math.cos(vec.x)))(Vec3.from(rot).multiply(radian(1)));
    }
